...
<body>	
	...
	<@ms.content>
		<@ms.contentBody>
			<@ms.contentNav title="版主类型管理">
				<@ms.savebutton id="saveFormButton" value="${name}"/>
				<@ms.contentNavBack value="返回版主类型列表"/>
			</@ms.contentNav>
				<@ms.contentPanel>
	    			<@ms.form isvalidation=true name="saveForm" style="width: 60%" class="form-inline" action="">
				    		<@ms.text name="categoryTitle" label="名称" title="名称" maxlength="30" style=" width:35%" placeholder="类别名称"  validation={"required":"true", "data-bv-notempty-message":"请填写栏目类别"}/>
				    		<@ms.textarea id="description" name="categoryDescription" label="描述:" style=" width:50%" title="描述" placeholder="栏目描述" maxlength="150"/>
				    		
				    		<input type="hidden" name="categoryId" id="categoryId" value="" disabled="true"/>
				    		<div class="form-group margin15">
			        			<label class="col-md-3 control-label col-xs-3">缩略图:</label>
	...
	$("#saveFormButton").click(function(){
		if($("#saveForm").data('bootstrapValidator').validate().isValid()){
			...
			$(this).request({url:"${base}${action}",data:formdata,type:"json",method:"post",func:function(msg) {
				if (msg.result) {
		     		alert("操作成功");
		    		location.href=base+"${baseManager}/bbs/forum/list.do?categoryCategoryId="+msg.resultData+"&modelId=${modelId}&modelTitle="+encodeURIComponent('${modelTitle?default("栏目")}');
		    	}else{
		    		alert(msg.resultMsg);
		    	}
			}});
