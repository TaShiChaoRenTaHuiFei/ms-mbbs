<@ms.html5>
    ...
    <@ms.panel>
	    			<@ms.form isvalidation=true name="saveForm" action="">
				    		<@ms.text name="categoryTitle" width="200" label="名称:" title="类别名称" maxlength="30"  placeholder="类别名称"  validation={"required":"true", "data-bv-notempty-message":"请填写栏目类别"}/>
				    		<@ms.textarea id="description" width="400" name="categoryDescription" label="描述:" title="栏目描述" placeholder="栏目描述" maxlength="150"/>
	...
	<#assign action="/manager/bbs/forum/save.do"/>
	<#assign name="添加"/>
	<#if category?has_content>
		<#assign action="/manager/bbs/forum/update.do"/>
		<#assign name="更新"/>
	</#if>
<script>
	//从后台得到值之后，显示在输入框中
